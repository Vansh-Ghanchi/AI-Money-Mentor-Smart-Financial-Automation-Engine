...
router.get('/stats', async (req, res) => {
    try {
        ...
        // Calculate financial throughput (last 30 days)
        const recentStats = await Transaction.aggregate([
            { $match: { date: { $gte: thirtyDaysAgo } } },
            { $group: { _id: '$type', total: { $sum: '$amount' } } }
        ]);

        const inflow = recentStats.find(s => s._id === 'income')?.total || 0;
        const outflow = recentStats.find(s => s._id === 'expense')?.total || 0;
        ...
        res.json({
            ...
            newUsersThisMonth,
            activeUsers,
            totalVolume: (inflow + outflow).toFixed(2),
...
router.get('/export/users/pdf', async (req, res) => {
    try {
        ...
        if (fs.existsSync(logoPath)) {
            doc.image(logoPath, 40, 25, { width: 30 });
            doc.fontSize(8).fillColor('#B4B4B4').font('Helvetica')
                .text('ExpenseAI', 75, 35);
        }

        // Title
        doc.fontSize(22).fillColor('#1E40AF').font('Helvetica-Bold')
            .text('User List Report', 0, 32, { align: 'center' });
        ...
        for (let i = range.start; i < range.start + range.count; i++) {
            ...
            doc.text(`Generated: ${new Date().toLocaleString()}  |  Page ${i + 1} of ${range.count}`, 40, pageHeight - 30, { align: 'center', width: 515 });
        }
...
router.get('/export/transactions/pdf', async (req, res) => {
    try {
        ...
        // Always Add App Logo (ExpenseAI) - Top Left branding
        const logoPath = path.join(__dirname, '../../frontend/public/assets/logo.png');
        if (fs.existsSync(logoPath)) {
            doc.image(logoPath, 40, 25, { width: 30 });
            doc.fontSize(8).fillColor('#B4B4B4').font('Helvetica')
        ...
        // Title
        doc.fontSize(22).fillColor('#1E40AF').font('Helvetica-Bold')
            .text('Transaction Report', 0, 32, { align: 'center' });
        ...
        for (let i = range.start; i < range.start + range.count; i++) {
            ...
            const pageHeight = doc.page.height;
            doc.fontSize(8).fillColor('#94A3B8').font('Helvetica');
            doc.text(`Generated: ${new Date().toLocaleString()}  |  Page ${i + 1} of ${range.count}`, 40, pageHeight - 30, { align: 'center', width: 760 });
        }
