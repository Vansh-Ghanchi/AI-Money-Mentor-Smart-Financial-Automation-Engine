        ...
        if (wallet) {
            if (type === 'income') {
                ...
                if (req.body.toWalletId) {
                    let toWalletQuery = { id: req.body.toWalletId };
                    if (user.businessId) {
                        toWalletQuery.businessId = user.businessId;
                    } else {
                        toWalletQuery.userId = req.userId;
                    }
                    const toWallet = await Wallet.findOne(toWalletQuery);
                    if (toWallet) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (newAmount !== oldAmount || newType !== oldType || req.body.walletId !== transaction.walletId) {
            let walletQuery = { id: transaction.walletId };
            if (user.businessId) {
                walletQuery.businessId = user.businessId;
            } else {
                walletQuery.userId = req.userId;
            ...
            if (wallet) {
                ...
                if (!req.body.walletId || req.body.walletId === transaction.walletId) {
                    ...
                    let newWalletQuery = { id: req.body.walletId };
                    if (user.businessId) {
                        newWalletQuery.businessId = user.businessId;
                    } else {
                        newWalletQuery.userId = req.userId;
                    }
                    const newWallet = await Wallet.findOne(newWalletQuery);
                    if (newWallet) {
                        if (newType === 'income') {
        ...
        if (wallet) {
            if (transaction.type === 'income') {
                ...
                if (transaction.toWalletId) {
                    let toWalletQuery = { id: transaction.toWalletId };
                    if (user.businessId) {
                        toWalletQuery.businessId = user.businessId;
                    } else {
                        toWalletQuery.userId = req.userId;
