...
import { Wallet, CreditCard, Banknote, Smartphone, Plus, X, ArrowLeft } from 'lucide-react';
import { format } from 'date-fns';

const Wallets = () => {
  const navigate = useNavigate();
  const { wallets, transactions, addWallet } = useExpense();
    ...
    if (newWalletName.trim()) {
      ...
      try {
        await addWallet({
          ...
          balance: 0,
          type: 'cash'  // Changed from 'other' to match backend enum
        });
        setNewWalletName('');
              ...
              <form id="add-wallet-form" onSubmit={handleAddWallet} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  <p className="text-[10px] text-slate-500 mt-2">Example: "Cash", "Main Savings", "UPI Wallet"</p>
                </div>
              </form>
            </div>
